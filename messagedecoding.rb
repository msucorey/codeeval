...
the same length, the second can be obtained from the first by adding 1
(base 2). Notice that there are no keys in the sequence that consist only of
1's.
...
is terminated by 000. Your program should accept the first argument as the
...
def decode(header, key)
  position = key.to_i(2) + offset(key.length)
  header[position]
end
...
lines.each do |line|
  ...
  line = (line.chars - header.chars).join
  message_complete = false
  while !message_complete
    set_header = ""
    set_complete = false
    set_header = line[0..2]
    key_length = set_header.to_i(2)
    line = line[3..-1]
    while !set_complete
      if !line[0..(key_length - 1)].include?("0")
        set_complete = true
        line = line[key_length..-1]
      else
        key = line[0..(key_length - 1)]
        message << decode(header, key)
        line = line[key_length..-1]
      end
    end
    message_complete = (line == "000")
  end
  puts message
end
